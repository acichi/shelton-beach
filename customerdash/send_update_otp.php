...
// Fetch destination details directly from customer table to avoid ID collisions in `users` view
$q = $conn->prepare("SELECT CONCAT(COALESCE(customer_fname,''),' ',COALESCE(customer_lname,'')) AS fullname, customer_email AS email, customer_number AS number, customer_user AS username FROM customer_tbl WHERE customer_id = ? LIMIT 1");
...
$_SESSION['profile_update_otp'] = $code;
$_SESSION['profile_update_otp_expires'] = time() + 300; // 5 minutes

// Persist to OTP table (best-effort)
@($conn->query("CREATE TABLE IF NOT EXISTS `otp_tbl` (
...
@($conn->query("CREATE TABLE IF NOT EXISTS `notifications_tbl` (
  ...
  `scope` enum('customer','admin','system') NOT NULL DEFAULT 'customer',
...
$ins = $conn->prepare('INSERT INTO otp_tbl (otp_code, customer_email, status, date_created, date_updated) VALUES (?, ?, "pending", NOW(), NOW())');
if ($ins) { $ins->bind_param('ss', $code, $u['email']); $ins->execute(); $ins->close(); }

if ($channel === 'phone' && !empty($u['number'])) {
    $msg = "Your Shelton verification code is: $code. This will expire in 5 minutes.";
    $res = sendSMS([$u['number']], $msg);
    if (($res['status_code'] ?? 0) >= 200 && ($res['status_code'] ?? 0) < 300) {
        // Log notification (SMS)
        $note = $conn->prepare("INSERT INTO notifications_tbl (user_id, scope, type, title, message, channel) VALUES (?, 'customer', 'otp', 'OTP sent', ?, 'sms')");
        if ($note) { $msgText = 'We sent an OTP to your phone ' . (string)$u['number']; $note->bind_param('is', $userId, $msgText); $note->execute(); $note->close(); }
        logActivity($conn, $userId, 'customer', 'otp_send', 'OTP sent via SMS');
...
$body    = '<p>Hello ' . htmlspecialchars($u['fullname'] ?? $u['username'] ?? 'User') . ',</p>' .
           '<p>Your verification code is:</p>' .
           '<p style="font-size:20px;font-weight:bold;letter-spacing:2px;">' . $code . '</p>' .
           '<p>This code expires in 5 minutes.</p>' .
           '<p>— Shelton</p>';
$html = renderEmailTemplate('Verification code', $body, ['preheader' => 'Verification code for account update']);
$alt  = "Your verification code is: $code";
$mail = sendEmail($u['email'], $subject, $html, $alt);

if ($mail['success'] ?? false) {
    // Log notification (Email)
    $note2 = $conn->prepare("INSERT INTO notifications_tbl (user_id, scope, type, title, message, channel) VALUES (?, 'customer', 'otp', 'OTP sent', ?, 'email')");
    ...
    logActivity($conn, $userId, 'customer', 'otp_send', 'OTP sent via Email');
